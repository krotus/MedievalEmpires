...
Public Class MedievalEmpires
    ...
    Public Sub populate()
        ...
        Try
            ...
            For i = 1 To 9
                ...
                For j = 0 To 50
                    Dim soldier As Soldier
                    If account.pEmpire.getTypeEmpire = "Roman" Then
                        soldier = legionary
                    ElseIf account.pEmpire.getTypeEmpire = "Gaul" Then
                        soldier = phalanx
                    ElseIf account.pEmpire.getTypeEmpire = "Teuton" Then
                        soldier = clubSwinger
                    End If
                    account.pEmpire.addSoldier(soldier)
                Next j
                Me.addUser(account)
            Next i
    ...
    Public Overloads Function getSoldiersByEmpire(empire As Empire) As List(Of Soldier)
    ...
    Public Overloads Function getSoldiersByEmpire(nomEmpire As String) As List(Of Soldier)
        Dim listSoldiers As New List(Of Soldier) From {}
        Dim eempire As Empire
        For Each emp In Me.empires
            For Each soldier In emp.pSoldiers
                If soldier.pEmpire.pName = nomEmpire Then
                    eempire = soldier.pEmpire
                    If soldier.pEmpire.GetType Is eempire.GetType Then
                        listSoldiers.Add(soldier)
                    End If
                End If
            Next
    ...
    Public Function getUserByUsername(ByVal username) As User
        Dim userResult As User
        For Each user In Me.pUsers
            If user.pUsername = username Then
                userResult = user
            End If
        Next
        Return userResult
